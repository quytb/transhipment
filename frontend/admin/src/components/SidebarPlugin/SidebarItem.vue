<template>
  <component :is="baseComponent"
    ...
    <a v-if="isMenu"
       ...
       @click="collapseMenu">
      <i :class="link.icon"></i>
      <p style="font-size: 20px">{{link.name}}
        <span v-if="!this.$sidebar.isMinimized" :class="collapsed ? 'ti-angle-down' : 'ti-angle-up'" id="icon_arrow_down"></span>
      </p>
    </a>
    ...
    <slot name="title" v-if="children.length === 0 && !$slots.default && link.path">
      <component
        ...
        <template v-if="addLink">
          <span class="sidebar-mini"><i :class="link.icon"></i></span>
