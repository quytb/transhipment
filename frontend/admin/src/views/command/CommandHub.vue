          ...
          <v-layout row wrap>
            ...
            <v-flex xs2>
              <v-autocomplete
                ...
                item-value="id"
                item-text="ten"
                label="Điểm trung chuyển"
                v-model="searchForm.currentHub"
              ></v-autocomplete>
            </v-flex>
            <v-flex xs2>
      ...
      <v-card-text>
        ...
        >
          <template v-slot:items="props">
            ...
            <td style="min-width: 150px;">{{ props.item.lyDoHuy }}</td>
            <td>
              <v-btn
                small
                class="margin0"
                title="Chi tiết lệnh"
                @click="editLenh(props.item.tcLenhId,props.item.taiXeTen)"
                icon
              ><v-icon small primary>ti-pencil-alt</v-icon></v-btn>
              <v-btn
                small
                class="margin0"
                icon
              ...
              ><v-icon small primary>delete</v-icon></v-btn>
            </td>
          </template>
        </v-data-table>
      </v-card-text>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                taskHeaders: [
                    ...
                    { text: 'Hub giao khách', align: 'left', sortable: false },
                    ...
                    { text: 'Thao tác', align: 'center', sortable: false }
                ],
                tasks : [],
        ...
        async mounted() {
            let [dsTaiXeRes,statusDieuLenhRes, dsLenhRes,listDiemTCDenRes] = await Promise.all([
            ...
            this.dsLenh   = dsLenhRes.data;
            this.dsDiemTC = listDiemTCDenRes.data;
        },
        watch : {
            pagination : {
                deep : true,
