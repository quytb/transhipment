    ...
    <v-card>
      ...
      <v-card-text>
        <div>
          ...
          ></v-autocomplete>
<!--          <span>Số khách/số ghế: {{soKhach}}/{{seats}}</span>-->
            </v-flex>
        </div>
        ...
        <div class="card width100">
          ...
          <div class="card-content" style="width: 100%">
            <v-radio-group v-model="ex7" class="width100" column>
              <el-collapse v-for="lenh in dsLenhDangCoCopy" :key="lenh.lenhId">
                <v-card class="mt-3 pa-0 elevation-2">
                  <v-card-text class="pa-3">
                    <v-radio
                      :label="`Lệnh id: ${lenh.lenhId} - ${lenh.trangThai} - Số khách/ghế: ${lenh.soKhach}/${lenh.seats}`"
                      color="red"
                      :value="lenh.lenhId"
                    ></v-radio>
...
export default {
  ...
  methods : {
    async fetchCustomers() {
      ...
      // this.soKhach = this.$axios.get(`/dieudo/tcd/dskhachdang${self.type == 'tcd' ? 'don' : 'tra'}/${this.ex7}`).data.veTrungChuyenDTOList.length;
